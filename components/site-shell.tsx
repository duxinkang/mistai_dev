    ...
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      >
        ...
        <nav className="hidden items-center gap-10 text-sm md:flex">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="transition hover:text-[#00a63e]">
              {item.label}
            </a>
          ))}
        </nav>
        <a
          ...
          className={`hidden rounded-md px-5 py-2 text-sm font-medium transition md:inline-flex ${
            scrolled
              ? "bg-[#00a63e] text-white hover:bg-[#008a34]"
              : "border border-white hover:border-[#9feec4] hover:bg-[#9feec4] hover:text-black"
          }`}
        >
          联系我们
        </a>
      ...
      <div className="relative z-10 mx-auto grid h-full max-w-[1440px] grid-cols-1 px-6 pt-28 pb-16 md:grid-cols-[1.1fr_1.4fr_0.8fr_0.7fr] md:px-16 md:pt-36">
        ...
        <div className="flex flex-col justify-center gap-7 md:gap-9">
          <h1 className="font-display animate-rise text-[clamp(2.7rem,8vw,4.65rem)] font-semibold leading-[1.08] tracking-[-0.03em] text-white">
            {hero.titlePrefix}
            <span className="inline-block text-[#9feec4]">
              {" "}
                ...
                <div>
                  ...
                  <div className="mb-3 flex flex-wrap gap-2">
                    {customer.tags.map((tag) => (
                      <span key={tag} className="rounded bg-green-100 px-2 py-1 text-xs font-medium text-green-600">
                        {tag}
                      </span>
                    ))}
                  </div>
...
function About() {
  ...
  return (
    <section className="section-shell">
      ...
      <div className="mx-auto h-1 w-[60%] overflow-hidden rounded-full bg-[#00a63e]/20">
        <div
          className="h-full rounded-full bg-[#00a63e] transition-transform duration-200"
        ...
        <div className="hide-scrollbar flex flex-col gap-5 overflow-x-auto">
          {techStacks.map((stack) => (
            <div key={stack.title} className="grid min-w-[960px] grid-cols-[84px_1fr] gap-4">
              <div className="rounded-md bg-[#00a63e] px-2 py-4 text-center text-sm font-medium tracking-[0.25em] text-white [writing-mode:vertical-rl]">
                {stack.title}
              </div>
      ...
      {fields.map((field) => (
        <label key={field.key} className="block">
          ...
          <input
            ...
            className="h-12 w-full rounded-md border border-slate-200 px-4 outline-none transition focus:border-[#00a63e] focus:ring-4 focus:ring-[#00a63e]/10 aria-[invalid=true]:border-[#ef4444] aria-[invalid=true]:ring-4 aria-[invalid=true]:ring-[#ef4444]/10"
          />
          {errors[field.key] ? <span className="mt-2 block text-sm text-[#ef4444]">{errors[field.key]}</span> : null}
        </label>
      ))}
      <button
        className="inline-flex h-12 items-center rounded-md bg-[#00a63e] px-6 font-medium text-white transition hover:bg-[#008a34] disabled:cursor-not-allowed disabled:bg-slate-400"
        type="submit"
        disabled={status === "submitting"}
      >
        {status === "submitting" ? "提交中..." : "提交"}
      </button>
      {message ? <p className={`text-sm ${status === "success" ? "text-[#00a63e]" : "text-[#ef4444]"}`}>{message}</p> : null}
    </form>
  );
}
